<template>
  <div class="login-container">
    ...
    <div class="login-inputBox">
      <div >
        <label>ID</label>
        <input id="login-id" name="username" type="text" v-model="id" v-on:keyup="{set_login_data_id}" autofocus>
        <div class="under-line"></div>
        <span :class="this.$store.state.signin_data.id.icon"></span>
      </div>
      <div>
        <label>PASSWORD</label>
        <input id="login-pw" name="password" type="password" v-model="pw" v-on:keyup="{set_login_data_pw}">
        <div class="under-line"></div>
      </div>
    </div>
...
<script>
    ...
    export default {
        ...
        computed:{
            set_login_data_id(){
                this.$store.state.signin_data.id.data = this.id;
                ...
                setTimeout(function () {
                    ...
                    if(self.$store.state.signin_data.id.count <= 0){
                        // console.log("set_login_data_id : sign_check_id 커밋 실행")
                        self.$store.commit("sign_check_id");
                    }
                },810);
                return this.$store.state.signin_data.id.data;
            },
            set_login_data_pw(){
                this.$store.state.signin_data.pw.data = this.pw;
